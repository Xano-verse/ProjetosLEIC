...
/**
 * Funcao publica do teste pratico. Dada uma matricula, procura as varias
 * instancias dela no sistema e para cada uma calcula a faturacao do parque
 * correspondente. Retorna a soma de todas estas faturacoes
 */
float faturacao_toda_dum_carro(char matricula[], Carro_ptr** hashtable) {
	...
	while(carro_ptr != NULL) {
		carro = (*carro_ptr).carro;

		if( !strcmp((*carro).matricula, matricula) \
		&& (*carro).data_hora_saida != NULL) {
			parque = procura_parque((*carro).nome_parque);
			faturacao_total += calcula_faturacao(parque, carro);
		}
