...
class UpdateEnrollmentMethodTest extends SpockTest {
    ...
    def setup() {
        ...
        shift.getStartTime() >> IN_ONE_DAY
        shift.getEndTime() >> IN_TWO_DAYS
    ...
    def "update enrollment"() {
        ...
        def shift3 = Mock(Shift)
        shift3.getActivity() >> activity
        shift3.getStartTime() >> IN_TWO_DAYS
        shift3.getEndTime() >> IN_THREE_DAYS
        shift3.getLocation() >> SHIFT_LOCATION

        when:
        enrollment.update(enrollmentDtoEdit, List.of(shift3))
        then: "checks results"
    ...
    def "try to update enrollment after deadline"() {
        ...
        when:
        enrollmentTwo.update(enrollmentDtoEdit, List.of(shift2))
