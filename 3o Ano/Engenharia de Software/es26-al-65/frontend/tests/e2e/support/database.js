...
const aBitAfterTomorrow = new Date(now);
aBitAfterTomorrow.setDate(now.getDate() + 1.5);
...
Cypress.Commands.add('createDatabaseInfoForEnrollments', () => {
  ...
  // Create 3 shifts  -  endTime comes first in the function
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(1, dayAfterTomorrow.toISOString(), tomorrow.toISOString(), 1, 1),
    credentials: credentials,
  })
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(5, dayAfterTomorrow.toISOString(), tomorrow.toISOString(), 1, 1),
    credentials: credentials,
  })
...
Cypress.Commands.add('createDatabaseInfoForParticipations', () => {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ACTIVITY_COLUMNS + generateActivityTuple(3, "A3", "Description A3", dayBeforeYesterday.toISOString(), yesterday.toISOString(),
      yesterday.toISOString(), 2, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(3, yesterday.toISOString(), yesterday.toISOString(), 1, 3),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_COLUMNS + generateEnrollmentTuple(5, 3),
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_SHIFTS_COLUMNS + " VALUES (5, 3)",
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_COLUMNS + generateEnrollmentTuple(6, 4),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_SHIFTS_COLUMNS + " VALUES (6, 3)",
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + PARTICIPATION_COLUMNS + generateParticipationTuple(7, 5, 3),
    credentials: credentials,
  })
});
...
Cypress.Commands.add('createDatabaseInfoForShifts', () => {
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ACTIVITY_COLUMNS + generateActivityTuple(1, "Shift Test Activity", "Activity for shift management testing", yesterday.toISOString(), yesterday.toISOString(),
      dayAfterTomorrow.toISOString(), 5, 1),
    credentials: credentials,
  })
});
