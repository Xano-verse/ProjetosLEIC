describe('Participation', () => {
  ...
  it('create participation', () => {
    ...
    cy.contains('.v-list-item', 'Lisbon').click();
  ...
  it('create participation on full shift', () => {
    const MEMBER_REVIEW = 'Activity was well organized';

    cy.intercept('GET', '/activities/3/enrollments').as('enrollments');
    ...
    // Second enrollment (the one without participation)
    cy.get('[data-cy="activityEnrollmentsTable"] tbody tr')
      .eq(1)
      .children()
      .eq(4)
    ...
    cy.contains('Selected shift is already full');
