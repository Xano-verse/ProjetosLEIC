describe('Enrollment', () => {
  ...
  it('create enrollment', () => {
    ...
    // select 2 shifts of the existent 3
    cy.get('[data-cy="shiftSelect"]').click()
    cy.get('.v-list-item:visible').eq(0).click();
    cy.get('.v-list-item:visible').eq(2).click();       // shifts appear sorted by time so 0 and 2 are not overlapped

    cy.get('[data-cy="saveEnrollment"]').click({force:true})
    cy.wait('@enroll')
    cy.logout()
  ...
  it('create enrollment with overlapping shifts', () => {
    const MOTIVATION = 'I am very keen to help other people.';
    ...
    cy.get('[data-cy="shiftSelect"]').click()
    cy.get('.v-list-item:visible').eq(0).click();
    cy.get('.v-list-item:visible').eq(1).click();         // shifts appear sorted by time so 0 and 1 are the overlapped ones
