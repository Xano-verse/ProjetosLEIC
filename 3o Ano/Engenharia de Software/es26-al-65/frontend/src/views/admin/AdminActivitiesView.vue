...
export default class AdminActivitiesView extends Vue {
  ...
  async created() {
    ...
    try {
      ...
      const flatActivities = this.institutions.flatMap(i => i.activities);
      const shifts = await Promise.all(flatActivities.map(a => RemoteServices.getActivityShifts(Number(a.id))));

      flatActivities.forEach((a, index) => {
