...
int valida_matricula(char matricula[]) {
	...
	/**
	 * Alteracao para teste pratico. Como podem existir 3 pares de digitos,
	 * os pares de letras podem ser 0, so os de numeros e que terao de ser
	 * diferentes de 0
	 */
	if(pares_nums == 0)
		return 0;
...
/**
 * Funcao do teste pratico. Verifica o erro que podera acontecer com
 * o comando u. Retorna 1 em caso de erro e 0 se nao houver erros.
 */
int erros_u(char matricula[]) {
...
/**
 * Funcao do teste pratico. Dada uma matricula, depois de verificar que esta e
 * valida, calcula e imprime o valor total pago pelo carro para todos os
 * parques do sistema.
 */
void u_comando(char* buffer, Carro_ptr** hashtable) {
	char matricula[COMP_MATRICULA+1];
	float valor_total_pago;
	
	sscanf(buffer+2, "%s", matricula);
	
	if(erros_u(matricula))
		return;

	valor_total_pago = faturacao_toda_dum_carro(matricula, hashtable);
	printf("%.2f\n", valor_total_pago);
}
...
float max_diario, int* num_parques_ptr) {
	/**
	 * Alteracao do teste pratico. Percorro o nome todo e caso encontre um
	 * digito sei que o nome nao e valido
	 */
	int i;
	for(i = 0; nome[i] != '\0'; i++) {
