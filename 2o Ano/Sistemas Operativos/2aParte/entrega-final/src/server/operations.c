...
int kvs_write(size_t num_pairs, char keys[][MAX_STRING_SIZE], char values[][MAX_STRING_SIZE]) {
	...
	for (size_t i = 0; i < num_pairs; i++) {	
        ...
        // Since keys are already sorted, we only lock the hashtable entry
        // if the key is different from the previous one
		if (i == 0 || strcmp(keys[i-1], keys[i]) != 0) {
			if (pthread_rwlock_wrlock(&kvs_table->rwlock[index]) != 0) {
				fprintf(stderr, "Failed to write-lock on write index %d\n", index);
			}
		}
	}
	...
	for (size_t i = 0; i < num_pairs; i++) {	
        ...
        // We only need to unlock the entries thar were locked
		if (i == 0 || strcmp(keys[i-1], keys[i]) != 0) {
			if (pthread_rwlock_unlock(&kvs_table->rwlock[index]) != 0) {
				fprintf(stderr, "Failed to unlock on write index %d\n", index);
			}
		}
	}
...
int kvs_read(size_t num_pairs, char keys[][MAX_STRING_SIZE], int fd) {
    ...
    for (size_t i = 0; i < num_pairs; i++) {
		...
		if (i == 0 || strcmp(keys[i-1], keys[i]) != 0) {
			...
			if (pthread_rwlock_rdlock(&kvs_table->rwlock[index]) != 0) {
		...
		if (i == 0 || strcmp(keys[i-1], keys[i]) != 0) {
			if (pthread_rwlock_unlock(&kvs_table->rwlock[index]) != 0) {
				fprintf(stderr, "Failed to unlock on read index %d\n", index); 
			}
		}
...
int kvs_delete(size_t num_pairs, char keys[][MAX_STRING_SIZE], int fd) {
	...
	// We lock the corresponding hashtable entry with a write lock since we'll be deleting from it
	for (size_t i = 0; i < num_pairs; i++) {
		int index = hash(keys[i]);
		...
		if (i == 0 || strcmp(keys[i-1], keys[i]) != 0) {
			if (pthread_rwlock_wrlock(&kvs_table->rwlock[index]) != 0) {
				fprintf(stderr, "Failed to write-lock on delete index %d\n", index);
			}
		}
	}
	...
	for (size_t i = 0; i < num_pairs; i++) {	
		...
		if (i == 0 || strcmp(keys[i-1], keys[i]) != 0) {
			if (pthread_rwlock_unlock(&kvs_table->rwlock[index]) != 0) {
				fprintf(stderr, "Failed to unlock on delete index %d\n", index);
			}
		}
	}
...
int kvs_show(int fd) {
    ...
    // We lock all the entries in the hashtable
	for (int i = 0; i < TABLE_SIZE; i++) {	
		pthread_rwlock_t *lockNode = &kvs_table->rwlock[i];
		
		if (pthread_rwlock_wrlock(lockNode) != 0) {
    ...
    // We unlock all the entries in the hashtable
	for (int i = 0; i < TABLE_SIZE; i++) {	
		pthread_rwlock_t *unlockNode = &kvs_table->rwlock[i];
		
		if (pthread_rwlock_unlock(unlockNode) != 0) {
			fprintf(stderr, "Failed to unlock index %d\n", i);
		}
	}
...
int add_client(char *key, int fd_notif) {
    return add_client_node(kvs_table, key, fd_notif);
}


int remove_client(char *key, int fd_notif) {
    return remove_client_node(kvs_table, key, fd_notif);
}


int kvs_set_global_client(char* key) {
	return set_global_client(kvs_table, key);
}


int kvs_advance_global_client(char* key) {
	return advance_global_client(kvs_table, key);
}
...
void kvs_remove_all_subscriptions() {
    remove_all_subscriptions(kvs_table);
}
