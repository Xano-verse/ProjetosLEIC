...
{ 
  ...
  start_button.position(width/2 - start_button.size().width/2, height/4 - start_button.size().height/2);
  
  // 4. Help text
  let texto = "Atenção!<br><br>O tempo só começa a contar depois do primeiro clique, por isso toma o teu tempo a analisar a disposição dos alvos e a memorizar onde está cada grupo.<br><br>As primeiras letras de cada palavra estão maiores e focar nessas ajuda.<br><br>Cada grupo está ordenado alfabeticamente por colunas.<br><br>Boa sorte!"
  help_text = createDiv(texto);
  help_text.style('color', 'white');
  help_text.style('font-family', 'Arial');
  help_text.style('width', '35%'); 
  help_text.style('text-align', 'left'); // align text
  help_text.position(width/50, height/2 + height/20);
//  textFont("Arial", 14);
